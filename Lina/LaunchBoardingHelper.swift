...
struct LaunchBoardingHelper {
    ...
    static func showOnboardingIfNeeded(in viewController: CreateArchiveViewController) {
        ...
        let page1 = LaunchBoardingPage.init(icon: page1icon.withRenderingMode(.alwaysTemplate), title: trans("Compression Support"), descriptionText: trans("Compress files using LZFSE or other methods for efficient storage and transfer."), showButton: false)
        let page2icon = UIImage(named: "signature") ?? UIImage()
        let page2 = LaunchBoardingPage.init(icon: page2icon.withRenderingMode(.alwaysTemplate), title: trans("Digital Signatures"), descriptionText: trans("Sign archives with ECDSA-P256 for authenticity and integrity verification."), showButton: false)
        ...
        let page3 = LaunchBoardingPage.init(icon: page3icon, title: trans("Open Source"), descriptionText: trans("Built on open standards and transparent cryptography."), showButton: false)
